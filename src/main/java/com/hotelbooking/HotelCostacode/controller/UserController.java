...
public class UserController {
    ...
    public ResponseEntity<Response> deleteMe() {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String email = auth.getName();
        Response response = userService.deleteMe(email);
        return ResponseEntity.status(response.getStatusCode()).body(response);
    }

    @PutMapping("/update-me")
    public ResponseEntity<Response> updateMyProfile(@RequestBody UserUpdateRequest request) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String email = auth.getName();
        Response response = userService.updateMyProfile(email, request);
        return ResponseEntity.status(response.getStatusCode()).body(response);
    }
